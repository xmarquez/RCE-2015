...
colonial <- colonial %>% group_by(ColRuler) %>% mutate(num = n())

ggplot()+ geom_map(aes(fill=reorder(ColRuler,desc(num)),map_id=country),map=world,data=colonial)+geom_path(data=world,aes(y=lat,x=long,group=group))+theme_bw() + theme(legend.position="bottom") + labs(fill="Primary colonial ruler",x="",y="")+guides(fill=guide_legend(title.position="top",ncol=3))
```
